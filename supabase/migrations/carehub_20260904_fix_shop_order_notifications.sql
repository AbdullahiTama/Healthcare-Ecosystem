-- ============================================================================
-- Fix: Shop order notifications should only fire after payment is confirmed
-- 
-- Bug: create_shop_order was sending notifications immediately at order creation
-- (status = pending_payment), before Paystack payment was confirmed.
-- 
-- Fix: Only send notifications for pay-at-pickup orders (no payment needed).
-- Paystack orders get notifications from verify-shop-payment.js / paystack-webhook.js
-- after payment is confirmed (shop_order_paid notification).
-- ============================================================================
...
AS $$
declare v_order_id uuid; v_order_ref text; v_item jsonb; v_ecom_id uuid; v_product_id uuid; v_qty int; v_ecom_price int; v_prod_price numeric; v_stock int; v_prod_name text; v_expected_price int; v_unit_price int; v_line_total int; v_existing uuid; v_ecom_status text; v_ecom_restricted boolean; v_ecom_business uuid; v_status text;
begin
  ...
  END LOOP;
  INSERT INTO shop_order_status_history (order_id, from_status, to_status, changed_by, note) VALUES (v_order_id, null, v_status, p_customer_id, 'Order created');
  ...
  -- Notifications: Only send for pay-at-pickup orders (payment_status = 'paid' at creation).
  -- Paystack orders (pending_payment) get notifications after payment confirmation via webhook/verify.
  IF p_delivery_preference = 'pickup' AND EXISTS (SELECT 1 FROM businesses WHERE id = p_vendor_business_id AND shop_allow_pay_on_delivery = true) THEN
    -- Pay at Pickup: notify vendor immediately (no payment needed)
    INSERT INTO staff_notifications (business_id, staff_id, is_owner, kind, title, body, link) SELECT p_vendor_business_id, s.id, false, 'shop_order', 'New Shop Order '||v_order_ref, 'Customer '||COALESCE(p_customer_name, p_customer_id::text)||' placed an order ('||(SELECT count(*)::text FROM jsonb_array_elements(p_items))||' items) — pay at pickup', '/dashboard/ecommerce/orders/'||v_order_id::text FROM staff s WHERE s.business_id=p_vendor_business_id AND s.status='active';
    INSERT INTO staff_notifications (business_id, staff_id, is_owner, kind, title, body, link) VALUES (p_vendor_business_id, null, true, 'shop_order', 'New Shop Order '||v_order_ref, 'Customer '||COALESCE(p_customer_name, p_customer_id::text)||' placed order '||v_order_ref||' — pay at pickup', '/dashboard/ecommerce/orders/'||v_order_id::text);
    INSERT INTO notifications (recipient_id, type, message, link) VALUES (p_customer_id, 'shop_order', 'Order '||v_order_ref||' placed — pay at pickup', '/orders/'||v_order_id::text);
  ELSE
    -- Paystack orders: only notify customer that order is pending payment (no vendor notification yet)
    INSERT INTO notifications (recipient_id, type, message, link) VALUES (p_customer_id, 'shop_order_pending', 'Order '||v_order_ref||' created — complete payment to confirm', '/orders/'||v_order_id::text);
  END IF;
  RETURN v_order_id;
end; $$;
