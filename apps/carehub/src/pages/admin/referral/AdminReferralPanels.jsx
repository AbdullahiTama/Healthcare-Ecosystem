        ...
        <Modal show onClose={() => setSelected(null)} sheet title={'Review — ' + selected.applicant_name}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {sameArea(selected).length > 0 && (
              <div style={{ fontSize: '12px', color: '#92400e', background: theme.warningBg, padding: '10px 12px', borderRadius: theme.radius.md }}>
    ...
    <div>
      ...
      {flags.data.length > 0 && (
        <Card style={{ padding: '14px', marginBottom: '14px', background: theme.warningBg, border: '1px solid #fcd34d' }}>
          <div style={{ fontWeight: '800', color: '#92400e', fontSize: '13px', marginBottom: '8px' }}>Payments that earned no commission (review needed)</div>
          {flags.data.slice(0, 8).map(f => (
            <div key={f.id} style={{ fontSize: '12px', color: '#92400e', fontFamily: theme.fontMono, padding: '3px 0' }}>{f.reason}{f.created_at ? ' · ' + fmtDate(f.created_at) : ''}</div>
          ))}
        </Card>
      )}
            ...
            <Card key={city} style={{ padding: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(230px,1fr))', gap: '10px' }}>
                {Object.keys(cities[city]).sort().map(area => {
                  ...
                  return (
                    <div key={area} style={{ padding: '12px', borderRadius: theme.radius.md, border: `1px solid ${ag.status === 'suspended' ? '#fcd34d' : theme.border}`, background: ag.status === 'active' ? theme.successBg : theme.gray50 }}>
                      <div style={{ fontSize: '13px', fontWeight: '700', color: navy }}>{area}
                        {pending > 0 && <span style={{ marginLeft: 6, fontSize: '11px', fontWeight: '700', color: '#92400e' }}>· {pending} application{pending > 1 ? 's' : ''}</span>}
                      </div>
