...
describe('categoryFromAmenity', () => {
  ...
  it('maps expanded healthcare facility categories (Manufacturer/Importer fix)', () => {
    ...
    expect(categoryFromAmenity('spa')).toBe(FACILITY_CATEGORY.SPA)
    expect(categoryFromAmenity('wellness')).toBe(FACILITY_CATEGORY.SPA)
...
describe('matchesCategory', () => {
  ...
  it('matches 16-category direct keys', () => {
    expect(matchesCategory({ category: FACILITY_CATEGORY.MANUFACTURER }, 'manufacturer')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.IMPORTER }, 'importer')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.DISTRIBUTOR }, 'distributor')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.SPA }, 'spa')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.COSMETICS }, 'cosmetics')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.DENTAL }, 'dental')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.EYE }, 'eye')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.PHYSIO }, 'physio')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.PRIMARY }, 'primary')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.AESTHETIC_CLINIC }, 'aesthetic')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.LABORATORY }, 'laboratory')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.MEDICAL_CENTRE }, 'medical_centre')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.SPECIALIST_CLINIC }, 'specialist')).toBe(true)
    expect(matchesCategory({ category: FACILITY_CATEGORY.HOSPITAL }, 'manufacturer')).toBe(false)
    expect(matchesCategory({ category: FACILITY_CATEGORY.SPA }, 'hospital')).toBe(false)
  })
})
