...
export function createCommentRepository({ client = supabase } = {}) {
  return {
    async getComments(postId) {
      const { data, error } = await client
        .from('post_comments')
        .select(`
          id, content, created_at, user_id, parent_id, mentions,
          profiles!user_id (id, display_name, full_name, is_verified, specialty, avatar_url),
          post_comment_likes (id, user_id)
        `)
        .eq('post_id', postId)
    ...
    async addCommentLike(commentId, userId) {
      const { data, error } = await client
        ...
        .select('id, comment_id, user_id').maybeSingle()
      if (error) throw error
      return data
    },
    ...
    async removeCommentLike(commentId, userId) {
      const { error } = await client
        .from('post_comment_likes').delete().eq('comment_id', commentId).eq('user_id', userId)
      if (error) throw error
    },
  }
}
