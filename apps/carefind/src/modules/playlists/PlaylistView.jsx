          ...
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            {isOwner && !isEditingTitle && (
              <>
                ...
                <button onClick={() => setConfirmDeletePartId(p.id)} style={{ background: theme.dangerBg, border: `1px solid ${theme.alert}`, borderRadius: 8, padding: '8px 9px', display: 'inline-flex', alignItems: 'center', cursor: 'pointer', color: theme.alert }} aria-label="Delete part"><Trash2 size={14} aria-hidden="true" /></button>
              </>
            )}
          </div>
        )
