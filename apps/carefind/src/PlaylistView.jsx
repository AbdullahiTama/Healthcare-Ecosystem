...
function PlaylistView() {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: theme.textLight }}>Loadingâ€¦</div>
  ...
  return (
    <div style={{ fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: 480, margin: '0 auto', paddingBottom: 90, background: '#fff', minHeight: '100vh' }}>
      <div style={{ background: theme.heroGradient, padding: '18px 16px', color: '#fff' }}>
        ...
        <div style={{ display: 'inline-block', background: 'rgba(255,255,255,0.15)', padding: '3px 10px', borderRadius: 12, fontSize: 10, fontWeight: 800, letterSpacing: '0.06em', marginBottom: 8 }}>ðŸŽ¬ PLAYLIST Â· {parts.length} PARTS</div>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: 12.5, color: 'rgba(255,255,255,0.75)' }}>
          By {playlist.profiles?.full_name || playlist.profiles?.display_name || 'CareFind'}{playlist.profiles?.is_verified && ' âœ“'}
        </p>
        ...
        <div style={{ padding: 18 }}>
          ...
          {(() => {
            ...
            if (part.kind === 'review') {
              ...
              return <div><p style={{ fontSize: 22, margin: '0 0 8px 0' }}>{'â­'.repeat(d.rating || 0)}{'â˜†'.repeat(5 - (d.rating || 0))}</p><p style={{ margin: 0, fontSize: 15, color: theme.navy, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{d.text}</p></div>
            }
          ...
          {hasNext ? (
            <button onClick={() => setCurrent(current + 1)} style={{ width: '100%', marginTop: 20, padding: 14, background: theme.tealGradient, color: '#fff', border: 'none', borderRadius: 14, fontWeight: 800, fontSize: 14, textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>â–¶ï¸ Up next: {parts[current + 1].title}</span>
              <span>â†’</span>
            </button>
          ) : (
            <p style={{ marginTop: 20, textAlign: 'center', fontSize: 13, color: theme.textLight, fontWeight: 600 }}>ðŸŽ‰ You've finished this series!</p>
          )}
        </div>
      )}
          ...
          return (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <button onClick={() => setCurrent(i)} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px', background: i === current ? theme.bg : '#fff', border: `1px solid ${theme.border}`, borderRadius: 10, textAlign: 'left', cursor: 'pointer' }}>
                ...
                {p.kind !== 'text' && <span style={{ fontSize: 14 }}>{p.kind === 'video' ? 'ðŸŽ¥' : p.kind === 'drawing' ? 'âœï¸' : p.kind === 'image' ? 'ðŸ–¼' : ''}</span>}
              </button>
              {isOwner && (
                <>
                  <button onClick={() => navigate(`/playlist/${id}/edit/${p.id}`)} style={{ background: theme.bg, border: `1px solid ${theme.border}`, borderRadius: 8, padding: '8px 9px', fontSize: 13 }}>âœï¸</button>
                  <button onClick={() => deletePart(p.id)} style={{ background: '#fef2f2', border: `1px solid ${theme.alert}`, borderRadius: 8, padding: '8px 9px', fontSize: 13 }}>ðŸ—‘</button>
                </>
              )}
            </div>
          )
