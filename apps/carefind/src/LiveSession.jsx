    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      {recording ? (
        ...
        <>
          ...
          <button onClick={sendVoice} style={{ padding: '6px 12px', background: theme.tealGradient, color: '#fff', border: 'none', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>Send ðŸŽ™ï¸</button>
          <button onClick={() => setAudioURL(null)} style={{ padding: '6px 8px', background: theme.bg, border: `1px solid ${theme.border}`, borderRadius: 20, fontSize: 12 }}>âœ•</button>
        </>
      ) : (
        <button onClick={startRec} style={{ padding: '6px 12px', background: theme.bg, border: `1px solid ${theme.border}`, color: theme.textMid, borderRadius: 20, fontSize: 12, fontWeight: 700 }}>ðŸŽ™ï¸ Voice</button>
      )}
    </div>
  )
}
    ...
    <div style={{ fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: 480, margin: '0 auto', paddingBottom: 80, background: '#f9fafb', minHeight: '100vh' }}>
      ...
      <div style={{ background: theme.heroGradient, padding: '14px 16px 14px', color: '#fff', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          <Link to="/" style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none', fontSize: 18 }}>â†</Link>
          <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            ...
            {host?.is_verified && host?.specialty && <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', marginLeft: 6 }}>âœ“ {host.specialty}</span>}
          </div>
          {/* Like count */}
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ fontSize: 13, transform: likeAnim ? 'scale(1.5)' : 'scale(1)', transition: 'transform 0.2s' }}>â¤ï¸</span>
      ...
      <div style={{ padding: '12px 12px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 10, padding: '10px 0', alignItems: 'center' }}>
          ...
          {!isHost && <span style={{ marginLeft: 'auto', fontSize: 11, color: theme.textLight }}>ðŸª™ {wallet} coins</span>}
        </div>

        {/* Gift panel */}
        {giftPanel && (
          <div style={{ border: `1px solid ${theme.border}`, borderRadius: 16, padding: 12, background: theme.cardBg, marginBottom: 10 }}>
            <p style={{ margin: '0 0 10px 0', fontWeight: 800, fontSize: 13, color: theme.navy }}>ðŸŽ Send a Gift (ðŸª™ {wallet} coins)</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
              {GIFT_TIERS.map(g => (
                <button key={g.label} onClick={() => sendGift(g)} disabled={wallet < g.coins}
                  ...
                  <p style={{ margin: 0, fontSize: 10, color: theme.tealDeep, fontWeight: 700 }}>ðŸª™ {g.coins}</p>
                </button>
              ))}
            </div>
          </div>
        ...
        <div style={{ border: `1px solid ${theme.border}`, borderRadius: 16, overflow: 'hidden', background: theme.cardBg, marginBottom: 10 }}>
          <div style={{ padding: '8px 12px', borderBottom: `1px solid ${theme.border}`, display: 'flex', justifyContent: 'space-between' }}>
            <p style={{ margin: 0, fontWeight: 800, fontSize: 13, color: theme.navy }}>ðŸ’¬ Live Chat</p>
          ...
          <div style={{ height: 220, overflowY: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {messages.length === 0 && <p style={{ color: theme.textLight, fontSize: 13, textAlign: 'center', margin: 'auto' }}>No messages yet â€” say hello! ðŸ‘‹</p>}
            {messages.map(m => (
              <div key={m.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <span style={{ fontSize: 11, fontWeight: 800, color: m.user_id === session.host_id ? theme.tealDeep : theme.navy }}>
                    ...
                    {m.user_id === session.host_id && ' ðŸŽ™ï¸'}
                  </span>
          ...
          {user && session.status === 'live' && (
            <div style={{ borderTop: `1px solid ${theme.border}`, padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <VoiceRecorder onSend={audioUrl => sendMessage('ðŸŽ™ï¸ Voice note', 'voice', audioUrl)} />
            </div>
          )}
