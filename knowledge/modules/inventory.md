...
## Summary figures (KPI row)
All six tiles above the product table come from one pure helper — `apps/carehub/src/modules/inventory/inventoryStats.js` `computeInventoryStats(products)`, unit-tested in `inventoryStats.test.js` and memoized in `Inventory.jsx`, so they recalculate live as stock changes. Definitions: **Stock Value** = Σ `price × stock`; **Cost** = Σ `cost_price × stock`; **Total Profit** = Stock Value − Cost; **Profit Margin %** = Profit ÷ Stock Value (the same selling-price basis as the per-product margin column, so the two are comparable), shown as `—` when nothing is in stock. Services (`cat`/`category === 'Services'`) are excluded from every money figure — they carry a sentinel stock of 999 and no cost price. Profit is *unrealised*: the margin still sitting on the shelf, not money earned — realised profit belongs to Reports. Because a product with no `cost_price` contributes its full selling value to profit, the tile's sub-line reports how many stocked items have no cost price, so an optimistic figure is visible as such rather than silently wrong. **Low Stock** counts non-service items with `0 < stock ≤ reorder_level` (default 5); **On CareFind** counts items with stock that are not opted out.
