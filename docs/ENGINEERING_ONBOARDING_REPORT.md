...
- **`products`** — written by CareHub's `Inventory.jsx`; read by CareFind's `Search.jsx`/`BusinessProfile.jsx`, filtered on a `list_on_carefind` boolean column. CareFind's queries also select several columns (`whatsapp`, `image_url`, `sale_type`, `price_unit`, `min_purchase`, `seller_location`) that do not appear anywhere in CareHub's own product-editing form (`Inventory.jsx`'s `ProductModal`) — no location in either codebase was found where these specific columns are written.
...
3. **What actually writes `products.whatsapp`, `image_url`, `sale_type`, `price_unit`, `min_purchase`, and `seller_location`?** These columns are read by CareFind but no write path for them was found in CareHub's product-management UI. Either such a path exists somewhere not covered by this review, or these fields are populated some other way not visible in either repository.
