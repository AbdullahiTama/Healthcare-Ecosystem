...
- **Second sanctioned use — CareFind's news reading surface** (`News.jsx`, `NewsArticle.jsx`): headline *and* body, because this is genuine long-form editorial reading rather than product UI, and a reading serif earns its keep there. Those two files previously hardcoded `Georgia, "Times New Roman", serif`, which meant the product shipped two different serifs depending on the screen; they now use the same `fontDisplay` token, so there is still exactly one serif in the system. Every non-editorial element on those screens (bylines, engagement bar, comments, the submit composer) stays on the sans stack.
