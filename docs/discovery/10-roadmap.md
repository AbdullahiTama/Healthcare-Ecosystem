...
- **Resolve the CareFind product-column gap** — decide where `whatsapp`/`image_url`/`sale_type`/`min_purchase`/`seller_location` are meant to be set, and build that path in CareHub's `Inventory.jsx`.**(`sale_type`/`price_unit`/`min_purchase` done)**** — the write path is now built: CareHub's inventory `ProductModal` exposes these fields with shared `@care-ecosystem/shared-marketplace` validation; the residual gap is now a decision on `whatsapp`, `image_url`, and `seller_location`.
