...
- [ ] **Repository-seam rollout (Candidate 1) still pending for ~23 modules.** `pos` has empty `repositories/hooks/components` scaffolding; `orders`/`clients` have unwired repositories+hooks their pages bypass; 18 modules import `services/supabase` directly. Apply the inventory pattern uniformly, or delete the unused scaffolding (see architecture review 2026-08-04).
