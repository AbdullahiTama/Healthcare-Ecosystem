...
- [ ] **Repository-seam rollout (Candidate 1) still pending for ~19 modules.** Done: `inventory`, `expenses`, `orders`, `clients`, `pos`. ~16 modules still import `services/supabase` directly. All module scaffolding is now either filled or deleted.
- [ ] **`updateDebt` is an id-only PATCH on the debts table.** Shared by `POS.jsx` (credit collection) and `Debts.jsx`, so it was left alone rather than changed from under a module that has not adopted the seam. Same latent class as the removed `updateSale`/`updateClient`: callers pass ids from business-scoped lists, so it is not leaking today. Scope it by `business_id` when `debts` adopts the seam. `purchaseRepository`/`debtRepository` still sit unused/misplaced in `inventory/repositories` — move them to `modules/purchases`/`modules/debts` as those adopt the seam. Apply the inventory/expenses/orders pattern uniformly, or delete the unused scaffolding (see architecture review 2026-08-04).
